...
RecursiveHTTPFetcher.prototype = {
    ...
    links: function(base_url, contents){
        ...
        anchors.forEach(function(link){
            ...
            if(link.match(/svnindex.xsl$/) || link.match(  /^(\w*:|)\/\//) || link.match(/^\./) ){
                ...
                links.push( (new include.File(base_url)).join(link) );
        } )
        return links;
    },
    push_d: function(dir){
        this.cwd = (new include.File(this.cwd)).join(dir);
        new include.File( this.cwd ).mkdir()
    },
    pop_d: function(){
        this.cwd = new include.File(this.cwd).dir();
    },
    download : function(link){
        //var text = readUrl( link);
        
        var bn = new include.File(link).basename();
        var f = new include.File(this.cwd).join(bn);
        ...
        new include.File(f).download_from( link, true );
    ...
    fetch_dir : function(url){
        this.level++;
        if(this.level > 0) this.push_d(  new include.File(url).basename() );
