    ...
    $recursivelyGatherTextFromNodeTree: function(aNode) {
		...
		set innerHTML(html){
		    ...
		    var doc = new DOMParser().
			  parseFromString('<div>'+html2xml(html)+'</div>');
...
__extend__(HTMLButtonElement.prototype, {
    ...
    set type(value){
        this.setAttribute('type',value) || 'input';
    },
    get value(){
...
var Event = function(options){
  ...
  this.currentTarget = options.currentTarget?options.currentTarget:null;
  this.target = options.target?options.target:document;
...
$w.dispatchEvent = function(event, bubbles){
    ...
    //calculate parents
    var current = event.target, parents = [this], i, bubble;
    
    if(bubbles)
        while((current = current.parentNode)) parents.push(current);
    

    /*if ( event.type && this.nodeType || this===window) {
        print("nodeType: " + this.nodeType);
    ...
    for(i  = 0; i < parents.length; i++){
    	bubble = parents[i];
    	if ( event.type && bubble.nodeType || bubble===window) {
